  ...
  function YouTubeAnalytics() {
    ...
    // New state for storing k-nearest neighbors (top 2)
    ...
    // Fetch sentiment analysis for the comparison video (similar caching can be applied here if desired)
    ...
    // New function: k-NN approach to get the top 2 similar comments (excluding target)
    const findKNearestComments = (k) => {
      ...
      const target = features[0];
      const featuresWithoutTarget = features.slice(1);
      // Compute distances for each comment
    ...
    // Handler for k-NN search button (for top 2 similar comments)
    const handleFindKNearestComments = () => {
      const result = findKNearestComments(2);
      setKNearestComments(result);
    };
          ...
          <div>
            ...
            {selectedComponents.includes("sentimentAnalysis") && (
              <div className="card mt-4 p-3">
                ...
                {/* New button for k-NN search (top 2 neighbors) */}
                <button className="btn btn-warning mt-3" onClick={handleFindKNearestComments}>
                  Find Top 2 Similar Comments
                </button>
                {kNearestComments.length > 0 && (
                  <div className="mt-2">
                    {kNearestComments.map((neighbor, index) => (
                      <div key={index}>
                        <p>
                          <strong>Comment {index + 1}:</strong> {neighbor.item.comment}
                        </p>
                        <p>
                          <strong>Distance:</strong> {neighbor.distance.toFixed(2)}
                        </p>
                      </div>
                    ))}
                  </div>
                  ...
                  <>
                    <ResponsiveContainer width="100%" height={350}>
                      <BarChart data={comparisonChartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                        ...
                        <Tooltip />
                        <Legend />
                        <Bar dataKey="primary" fill="#8884d8" name={videoTitle || "Primary Video"} />
                        <Bar dataKey="comparison" fill="#82ca9d" name={compareVideoData.title || "Comparison Video"} />
                      </BarChart>
                    </ResponsiveContainer>
                    <div className="row mt-4">
                      ...
                      <div className="col-md-6">
                        ...
                        <ResponsiveContainer width="100%" height={350}>
                          <PieChart>
                            <Pie data={compareSentimentChartData} dataKey="value" cx="50%" cy="50%" outerRadius={100} label>
